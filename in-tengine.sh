...
./configure --prefix=/server/tengine --with-pcre=/tmp/pcre-8.00 --with-http_sub_module --with-http_gzip_static_module  --with-ld-opt="-Wl,-rpath,/usr/local/lib"  --with-http_lua_module --with-luajit-inc=/usr/local/include/luajit-2.0 --with-luajit-lib=/usr/local/lib --add-module=/tmp/ngx_devel_kit-0.2.17rc2
make && make install
cd ..
